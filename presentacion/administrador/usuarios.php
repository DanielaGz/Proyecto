...
<script>
    function buscar() {
        var url = "indexAjax.php?pid=<?php echo base64_encode("presentacion/administrador/usuariosAjax.php") ?>" + "&filtro=" + $("#busqueda").val();
        $("#logs").load(url);
    }

    function ver(v) {
        var url = "indexAjax.php?pid=<?php echo base64_encode("presentacion/administrador/usuarioModal.php") ?>" + "&id=" + v;
        $("#ver").load(url);
        $('#user').modal('show');
    }
